...
def get_arguments():
    parser = argparse.ArgumentParser(description='Creates a 360° saliency map for the given SMILES string')
...
saliency_map_calc = saliency_map.SaliencyMap(network_path, 0)
with progressbar.ProgressBar(360) as progress:
    for i in range(360):
        ...
        grads = saliency_map_calc.calculate(tensor[0])
        saliency_map_data = grads[:] * prediction
        heatmap = saliency_map.array_to_heatmap([saliency_map_data])[0]
