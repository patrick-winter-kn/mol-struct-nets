...
class RandomSmiles:
    ...
    def execute(global_parameters, parameters):
        # TODO don't write into same file
        ...
        if file_util.file_exists(data_set_path):
            ...
            chunks = misc.chunk(parameters['n'], thread_pool.number_threads)
            checker = duplicate_checker.DuplicateChecker()
            with multithread_progress.MultithreadProgress(parameters['n']) as progress:
                with thread_pool.ThreadPool() as pool:
                    for chunk in chunks:
                        generator = smiles_generator.SmilesGenerator(chunk['size'], parameters['max_length'], global_parameters['seed'], chunk['start'], progress, checker)
