...
class RenderSmilesAttention:
    ...
    def execute(global_parameters, local_parameters):
        ...
        if file_structure.AttentionMap.attention_map_active in attention_map_h5.keys():
            ...
            RenderSmilesAttention.render(attention_map_active, indices, smiles, active_dir_path)
        if file_structure.AttentionMap.attention_map_inactive in attention_map_h5.keys():
            ...
            RenderSmilesAttention.render(attention_map_inactive, indices, smiles, inactive_dir_path)
        attention_map_h5.close()
        data_h5.close()


    @staticmethod
    def render(data_set, indices, smiles, output_dir_path):
        if indices is None:
            indices = range(data_set)
