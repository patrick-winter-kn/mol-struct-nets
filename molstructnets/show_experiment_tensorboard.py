...
def get_arguments():
    parser = argparse.ArgumentParser(description='Shows the tensor board of an existing experiment')
...
for i in range(nr_steps):
    ...
    if type_id == partitioning_repository.instance.get_id():
        global_parameters[constants.GlobalParameters.partition_data] = \
            file_util.get_filename(step.get_result_file(global_parameters, parameters), False)
tensor_board_path = file_structure.get_network_file(global_parameters)
tensor_board_path = tensor_board_path[:tensor_board_path.rfind('.')] + '-tensorboard'
threading.Thread(target=open_tensorboard).start()
os.system(executable_path + 'tensorboard --logdir=' + tensor_board_path)
