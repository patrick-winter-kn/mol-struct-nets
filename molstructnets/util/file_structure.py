...
def get_preprocessed_folder(global_parameters):
    return file_util.resolve_subpath(global_parameters['root'], ['data_sets', global_parameters['data_set'],
                                                                 'preprocessed'])


def get_network_file(global_parameters):
    return file_util.resolve_subpath(global_parameters['root'],
                                     ['experiments', global_parameters['experiment'], global_parameters['data_set'],
                                      global_parameters['target'], 'network.h5'])


def get_prediction_file(global_parameters):
    return file_util.resolve_subpath(global_parameters['root'],
                                     ['experiments', global_parameters['experiment'], global_parameters['data_set'],
                                      global_parameters['target'], 'predictions.h5'])
